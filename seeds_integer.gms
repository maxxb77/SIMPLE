...
binary variable 
       on(i,v,r,h,t) "1 if unit is on during time period h",
       start(i,v,r,h,t) "1 if unit is started in time period h"
;
...
* available amount of capacity
        ...
        * ( 1$(not min_up_time(i)) + on(i,v,r,h,t)$min_up_time(i) )
...
* solve the model
    solve seeds using minlp minimizing Z ;
) ;
